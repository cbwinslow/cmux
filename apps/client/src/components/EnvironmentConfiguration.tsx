...
const generateEnvVarRowKey = (): string => {
  if (
  ...
  return `env-var-${Date.now()}-${Math.random().toString(36).slice(2)}`;
};

const ensureEnvVarRowKey = (row: EnvVarWithRowKey): string => {
  if (!row.__rowKey) {
    row.__rowKey = generateEnvVarRowKey();
  }
  return row.__rowKey;
};
            ...
            >
              ...
              <div className="space-y-2">
                {envVars.map((row, idx) => {
                  ...
                  const rowKey = ensureEnvVarRowKey(envRow);
                  return (
                    <div
                      key={rowKey}
                    ...
                    >
                      ...
                      <TextareaAutosize
                        ...
                        placeholder="example-secret-value"
                        minRows={1}
                        maxRows={10}
